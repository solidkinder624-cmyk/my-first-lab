...
## GRAZE & GRACE ― スワイプ弾幕壁 ＆ 回避シールド ＆ 芸術点 プロトタイプ (games/graze-and-grace/)

企画書 (`games/graze-and-grace/GAME_DESIGN.md`) の Phase 1「弾幕エンジンの基礎構築」、
Phase 2「回避側（勇者）のプロトタイプ実装」、Phase 3「評価システム（Grace & Graze）の
スコア計算ロジック」に対応するプロトタイプ。外部ライブラリなしの HTML5 Canvas 製。
...
# 弾幕の壁 生成ロジック / シールドシステム / スコア計算だけを Node で検証する
...
画面中央側へ直交する方向へ壁全体が一斉に飛び出す。弾をカスるたびにシールドゲージが
少しずつ溜まり、MAXになると `Space` で一定時間（1.5秒）の無敵状態を発動できる。
発動回数は企画書 3.1 のとおり1プレイにつき5回までで、使い切るとゲージがMAXでも
発動できなくなる。

画面下部の「芸術点」パネルには、企画書 2.1 の Grace（規則性・密度・継続性・
美しさ＝画面の使い方）と Graze（連続カスリ）から算出したスコアをリアルタイムに
表示する（シンクロ率は音楽同期を実装する Phase4 で追加予定のため未実装）。
...
- スコア計算（`score.js`）は壁の履歴とカスリ/被弾イベントの時系列を受け取り、
  規則性・密度・継続性・カバレッジ(美しさの下敷き)を 0〜100 に正規化したうえで
  Graceスコアとし、連続カスリ数にストリークボーナスを加えた Graze スコアと
  組み合わせて芸術点を出す。被弾はどのスコアにも加点しないため、企画書の
...
- 当初 React での実装を想定していたが、このサンドボックス環境では CDN
  ...
  `score.js` はフレームワーク非依存の純関数なので、React/Unity/Godot 等へ
  載せ替える場合もロジックはそのまま流用できる。
