...
## マイクラ風ボクセルゲーム "CUBE WORLD" (games/minecraft/)

外部ライブラリなしの WebGL 製。`games/minecraft/index.html` をブラウザで開く
だけで遊べる（ビルド不要・ファイルを直接開いてOK）。地形は無限に自動生成され、
掘る / 積むの結果は `localStorage` に自動保存される。

```bash
# ローカルで開く
xdg-open games/minecraft/index.html    # macOS なら open

# ワールドと物理が壊れていないかを検証する（後述）
...
| 入力 | 動作 |
|---|---|
| `W A S D` | 移動　`Ctrl` ダッシュ　`Shift` しゃがむ / 飛行中は降下 |
| `Space` | ジャンプ（水中では浮上）／ 2 回押しで飛行 ON/OFF（`F` でも同じ） |
| マウス移動 | 視点（画面クリックでポインタロック） |
| 左ボタン長押し | ブロックを壊す（硬さぶんの時間がかかる） |
| 右クリック | 持っているブロックを置く　中クリック: 見ているブロックを持つ |
| `1`〜`9` / ホイール | ホットバーの持ち替え |
| `R` | スポーン地点へ戻る　`F3` デバッグ表示　`M` サウンド　`ESC` ポーズ（自動セーブ） |

スマホ・タブレットでは左下のスティックで移動、画面ドラッグで視点、
長押しで採掘、`PUT` / `JUMP` / `FLY` ボタンで設置・ジャンプ・飛行。

### ゲーム内容

- シードから決まる無限地形（大陸・山脈・海・砂浜・雪山・洞窟・木）を
  チャンク単位で生成し、プレイヤーの周囲だけを読み込む
- 15 種類のブロック。壊す・置く・水に潜る・飛ぶができるクリエイティブ寄りの遊び方
- 480 秒で 1 日が巡る昼夜サイクル。ランプブロックは半径 7 ブロックを
  暖色で照らし、夜でも時間帯の色に影響されない
- ブロックの改変だけを差分としてセーブするので、
  無限に広い世界でもセーブデータは触った場所のぶんしか増えない
- タイトルでシード（数値でも文字列でも可）と描画距離を指定できる

### 「壊れた世界」を防ぐ検証器

`verify.mjs` は `index.html` の `CORE-BEGIN` 〜 `CORE-END` で囲まれた
**ゲーム本体とまったく同じロジック**（ワールド生成・当たり判定・レイキャスト・
セーブ形式）だけを切り出し、Node 上で 12 項目を自動チェックする。

```bash
node games/minecraft/verify.mjs
```

- 同じシードなら必ず同じ地形になるか／**チャンクの生成順を変えても同じか**
  （木はチャンクをまたいで生えるため、順序に依存すると継ぎ目が壊れる）
- 岩盤・海面・高さが仕様の範囲に収まっているか
- スポーン地点が本当に立てる場所か（足元が地面で頭上が空いているか）
- 上空から落として着地するか、走り回ってブロックをすり抜けないか
  （AABB を少し縮めた判定で「めり込み」を検出）
- DDA レイキャストが 0.002 刻みの総当たり計算と同じブロック・同じ面を返すか
- セーブ → ロードでブロックの改変が完全に復元されるか

### 実装メモ

- **ロジックと描画の分離**: `CORE-BEGIN`〜`CORE-END` は DOM にも WebGL にも
  触らない純粋関数群。検証器がゲーム本体と同じコードを再利用できるのは
  この分離のおかげで、ジオメトリーダッシュ (`games/geometry-dash/`) と同じ方針
- **チャンク**: 16×80×16 を 1 単位として生成・メッシュ化。1 フレームあたりの
  生成 2 個 / メッシュ化 3 個に制限して、移動中もカクつかないようにしている
- **面の間引き**: 隣が不透明ブロックの面は頂点を作らない。頂点ごとに
  周囲 3 ブロックからアンビエントオクルージョンを計算し、
  明暗差が小さくなる向きに四角形を三角形分割して縞模様を防いでいる
- **明るさ**: 列ごとの「空が見える高さ」からの深さで減衰させる簡易スカイライト
  ＋ ランプの点光源。ランプは各チャンクが座標リストを持ち、周囲 3×3 チャンク
  ぶんだけを見るので、ランプが無い場所は完全に無コスト
- **テクスチャ**: 16×16 タイル 16 枚を起動時に Canvas 2D で描いて 1 枚の
  アトラスにまとめる（画像ファイルなし）。ホットバーのアイコンも同じタイルを
  アフィン変換して立方体に見せている
- **物理**: 固定タイムステップ 1/120 秒。軸ごとに動かして、めり込んだら面まで
  押し戻す方式なので、高速移動でもブロックをすり抜けない
- **音**: WebAudio でノイズとフィルタからその場で合成（音源ファイルなし）
