...
画面中央側へ直交する方向へ壁全体が一斉に飛び出す。BGMが鳴っている間はこの予告時間が
自動調整され、壁は必ず次の拍にピッタリ合わせて発射される。弾をカスるたびにシールド
ゲージが少しずつ溜まり、MAXになると `Space` で一定時間（1.5秒）の無敵状態を発動できる
（企画書3.1のとおり1プレイ5回まで）。

画面下部の「芸術点」パネルには、企画書 2.1 の Grace（規則性・密度・継続性・美しさ・
シンクロ率）と Graze（連続カスリ）から算出したスコアをリアルタイムに表示する。
...
- スコア計算（`score.js`）は壁の履歴とカスリ/被弾イベントの時系列を受け取り、
  規則性・密度・継続性・カバレッジ(美しさの下敷き)・シンクロ率を 0〜100 に
  正規化したうえで Graceスコアとし、連続カスリ数にストリークボーナスを加えた
  Graze スコアと組み合わせて芸術点を出す。被弾はどのスコアにも加点しないため、
  企画書の「絶対に避けられない壁による瞬殺は評価されない」は自然に満たされる。
- 拍クロック（`rhythm.js`）は `{bpm, beatsPerBar, startTime}` を受け取る
  `createClock` と、そこから拍インデックス・拍の位相(0〜1)・次の拍の時刻・
  拍への同期精度(`syncAccuracy`)を計算する純関数群。AudioContext 等には
  一切依存しないので Node で単体テストできる。`index.html` 側では
  `AudioContext.currentTime` を起点にクロックを作り、`performance.now()`
  ベースの `requestAnimationFrame` ループとの時間軸のズレは
  `audioOffsetMs`（音楽開始時に一度だけ測る差分）で吸収している。
  スワイプ完了時、次の拍までの時間を予告時間として渡すことで壁の発射タイミングを
  拍に量子化し、その瞬間の `syncAccuracy` を芸術点の「シンクロ率」サンプルとして
  記録する。BGM自体は音源ファイルを使わず、拍ごとに WebAudio の
  `OscillatorNode`＋`GainNode` で短いクリック音をその場で合成している
  （小節の頭だけ音を高くして拍の区別をつけている）。
- ソロモードのAI（`ai.js`、企画書4.1）は3種類とも `decideMove(type, hero, bullets, opts)`
  という同じ形の純関数で、キーボード入力の dx/dy と同じ単位ベクトルを返すだけなので
  既存の移動処理にそのまま差し込める。`ビギナー型`は近い発射済み弾から単純に反発するだけ
  （安全地帯を探さないので追い詰められやすい）、`スコアラー型`は最寄りの弾との距離を見て、
  遠ければ自分から近づき、被弾しそうなら退避し、ちょうど良い距離では静止してカスリを
